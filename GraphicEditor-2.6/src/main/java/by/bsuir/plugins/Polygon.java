...
public class Polygon implements MyShape {
    ...
    public Polygon(Point center, double radius, int n, PolygonType polygonType) {
        ...
        polygonPoints(this.center, this.radius, this.n, this.polygonType, reflectionType);
    }

    public Polygon(Point center, double radius, int n, PolygonType polygonType, Color color) {
        this();
        this.color = color;
        this.radius = radius;
        this.center = center;
    ...
    public Polygon(Point center, double radius, int n, PolygonType polygonType, ReflectionType reflectionType) {
        this();
        this.color = Color.BLACK;
        this.radius = radius;
        this.center = center;
        ...
        polygonPoints(this.center, this.radius, this.n, this.polygonType, this.reflectionType);
    }

    public Polygon(Point center, double radius, int n, PolygonType polygonType, ReflectionType reflectionType, Color color) {
        this();
        this.color = color;
        this.radius = radius;
        this.center = center;
    ...
    private void polygonPoints(Point center, double radius, int n, PolygonType polygonType, ReflectionType reflectionType) {
        ...
        if (polygonType.equals(PolygonType.ISOSCELES_TRIANGLE) || polygonType.equals(PolygonType.REGULAR_POLYGON)) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                // if reflection type is not specified
                if (reflectionType.equals(ReflectionType.NONE) || reflectionType.equals(ReflectionType.UP)) {
                    rotatedAngle = 3 * Math.PI / 2;
                } else if (reflectionType.equals(ReflectionType.DOWN)) {
                    rotatedAngle = Math.PI / 2;
                } else if (reflectionType.equals(ReflectionType.LEFT)) {
                    rotatedAngle = 2 * Math.PI / 2;
                } else if (reflectionType.equals(ReflectionType.RIGHT)) {
                    rotatedAngle = 4 * Math.PI / 2;
                }
